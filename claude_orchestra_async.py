...
5. Output: TASKS_ADDED: <list>, CODEBASE_HEALTH: <assessment>"""
    ...
    async def run_pipeline(self, parallel: bool = True) -> Dict[str, AgentResult]:
        ...
        # Stage 3: Reviewer
        # OPTIMIZED: Run reviewer first, then conditionally run planner
        # This saves ~1,800-3,500 tokens on failed cycles
        logger.info("\n[3/4] Reviewer")
        review = await self.run_reviewer(pr_number=test.pr_number)
        results['reviewer'] = review

        # Parse review decision from output
        review_approved = False
        if review.success and review.output:
            output_upper = review.output.upper()
            if "REVIEW_DECISION: APPROVED" in output_upper or "REVIEW_DECISION:APPROVED" in output_upper:
                review_approved = True

        # Stage 4: Planner (only on approved PRs to save tokens)
        if review_approved:
            logger.info("\n[4/4] Planner")
            results['planner'] = await self.run_planner()
        else:
            logger.info("\n[4/4] Skipping Planner (PR not approved - saving tokens)")
            results['planner_skipped'] = True

        # Update state
        self.state.last_run = datetime.now().isoformat()
        self.state.total_cycles += 1
        # Only check AgentResult objects for success (skip planner_skipped bool)
        agent_results = [r for r in results.values() if isinstance(r, AgentResult)]
        if all(r.success for r in agent_results):
    ...
    def _print_summary(self, results: Dict[str, AgentResult]):
        # Filter to only AgentResult objects
        agent_results = {k: v for k, v in results.items() if isinstance(v, AgentResult)}
        total_time = sum(r.duration_seconds for r in agent_results.values())
        print("\n" + "=" * 50)
        print("📊 Pipeline Summary")
        print("-" * 50)
        for name, r in results.items():
            if name == 'planner_skipped':
                print("⏭ PLANNER: Skipped (PR not approved)")
                continue
            status = "✓" if r.success else "✗"
            print(f"{status} {name.upper()}: {r.duration_seconds:.1f}s")
        print("-" * 50)
        success_count = sum(1 for r in agent_results.values() if r.success)
        print(f"Total: {total_time:.1f}s | Success: {success_count}/{len(agent_results)}")
